...
class Qnetwork:
    ...
    def __init__(self, myScope, agent, env, batch_size, gamma, trace_length, hidden, simple_net, lr):
        ...
        with tf.variable_scope(myScope):
            ...
            self.sample_return_Avalue = tf.placeholder(shape=[None, trace_length],dtype=tf.float32, name='sample_return_Avalue')
            self.sample_return_Svalue = tf.placeholder(shape=[None, trace_length],dtype=tf.float32, name='sample_return_Svalue')
            self.sample_reward = tf.placeholder(shape=[None, trace_length], dtype=tf.float32, name='sample_reward_new')

            self.next_value = tf.placeholder(
                shape=[None, 1], dtype=tf.float32, name='next_value')
            self.next_v = tf.matmul(self.next_value, self.gamma_array_inverse)
            self.target_Svalue = self.sample_return_Svalue + self.next_v
            self.target_Avalue = self.sample_return_Avalue + self.next_v

            self.actions = tf.placeholder(shape=[None], dtype=tf.int32)
            self.actions_onehot = tf.one_hot(
                self.actions, env.NUM_ACTIONS, dtype=tf.float32)

            self.actions_past = tf.placeholder(shape=[None], dtype=tf.int32)
            self.actions_past_onehot = tf.one_hot(
                self.actions_past, env.NUM_ACTIONS, dtype=tf.float32)
            ...
            self.log_pi_action = tf.reduce_mean(
                ...
                reduction_indices=1)
            self.log_pi_action_past = tf.reduce_mean(
                tf.multiply(self.log_pi, self.actions_past_onehot),
                reduction_indices=1)

            #self.total_target = self.target_Svalue + self.target_Avalue
            error__current = tf.square(self.target_Svalue - self.value) / 2
            #error__future = tf.square(self.target_Avalue - self.value_future) / 2

            self.td_error = error__current #+ error__future
            self.loss = tf.reduce_mean(self.td_error)
        ...
        self.log_pi_action_bs_past = tf.reduce_sum(tf.reshape(self.log_pi_action_past, [-1, trace_length]),1)
        self.log_pi_action_bs_t_past = tf.reshape(self.log_pi_action_past, [batch_size, trace_length])

        self.setparams= SetFromFlat(self.parameters)
        self.getparams= GetFlat(self.parameters)
        self.getQvalueParams= GetFlat(self.value_params)
...
class ExperienceBuffer:
    ...
    def sample(self,batch_size,trace_length):
        ...
        return np.reshape(sampledTraces,[batch_size*trace_length,8])
