...
def update(mainQN, lr, final_delta_1_v, final_delta_2_v):
    #pdb.set_trace()
    print("\t\t>> GRADS:", final_delta_1_v, final_delta_2_v)
...
def train(env, *, num_episodes, trace_length, batch_size, gamma,
    ...
    summaryLength = 1 #Number of epidoes to periodically save for analysis
    #summaryLength = batch_size #Number of epidoes to periodically save for analysis
    ...
    with tf.Session() as sess:
        ...
        for i in range(num_episodes):
            ...
            while j < max_epLength:
                ...
                for agent_role, agent in enumerate(these_agents):
                    episodeBuffer[agent_role].append([
                        ...
                        d, these_agents[agent_role], a_all_old[agent_role], r_old[agent_role]
                    ])
            ...
            if (episodes_run[agent] % batch_size == 0 and
                ...
                sample_return_Svalue0 = np.reshape(get_monte_carlo_Svalue(trainBatch0[:,2], y, trace_length, batch_size), [batch_size, -1])
                sample_return_Svalue1 = np.reshape(get_monte_carlo_Svalue(trainBatch1[:,2], y, trace_length, batch_size), [batch_size, -1])
                sample_return_Avalue0 = np.reshape(get_monte_carlo_Avalue(trainBatch0[:,7], y, trace_length, batch_size), [batch_size, -1])
                sample_return_Avalue1 = np.reshape(get_monte_carlo_Avalue(trainBatch1[:,7], y, trace_length, batch_size), [batch_size, -1])
                ...
                feed_dict = {
                    ...
                    mainQN[0].sample_return_Avalue: sample_return_Avalue0,
                    mainQN[1].sample_return_Avalue: sample_return_Avalue1,
                    #mainQN[0].sample_return: sample_return0,

                    mainQN[0].actions: trainBatch0[:,1],
                    mainQN[0].actions_past: trainBatch0[:,6],

                    mainQN[1].scalarInput: np.vstack(trainBatch1[:,0]),

                    mainQN[0].sample_return_Svalue: sample_return_Svalue0,
                    mainQN[1].sample_return_Svalue: sample_return_Svalue1,
                    #mainQN[1].sample_return: sample_return1,

                    mainQN[1].actions: trainBatch1[:,1],
                    mainQN[1].actions_past: trainBatch1[:,6],
                ...
                if episodes_run[agent]  %  batch_size == 0 and  episodes_run[agent] > 0:
                    values, _, _, update1, update2, grad_1, grad_2 = sess.run(fetches, feed_dict=feed_dict)
            ...
            if len(rList) % summaryLength == 0 and len(rList) != 0 and updated == True:
                ...
                log_items['ndr_agent0'] = log_items['reward_agent0'] * (1-gamma)
                log_items['ndr_agent1'] = log_items['reward_agent1'] * (1-gamma)
                ...
                if simple_net:
                    ...
                    qvalues_1_vals =  mainQN[0].getQvalueParams()
                    qvalues_2_vals =  mainQN[1].getQvalueParams()
                    print('qvalues_1_vals', qvalues_1_vals)
                    print('qvalues_2_vals', qvalues_2_vals)


                    for __index in range(len(theta_1_vals)):
                        log_items['theta_1_' + str(__index)] = theta_1_vals[__index]
                    for __index in range(len(theta_2_vals)):
                        log_items['theta_2_' + str(__index)] = theta_2_vals[__index]
                    for __index in range(len(qvalues_1_vals)):
                        log_items['qvalues_1_' + str(__index)] = qvalues_1_vals[__index]
                    for __index in range(len(qvalues_2_vals)):
                        log_items['qvalues_2_' + str(__index)] = qvalues_2_vals[__index]
