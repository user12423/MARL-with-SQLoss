...
class IteratedPrisonersDilemma(gym.Env):
    ...
    def __init__(self, max_steps):
        ...
        #self.payout_mat = np.array([[1., 2.8], [-1.0, 0.0]])    ## Coin-game, Sticky=4, Lookahead=4
        self.payout_mat = np.array([[-1., 0.], [-3., -2.]])                ### Original IPD
        #self.payout_mat = np.array([[4., 3.], [0., 1.]])                 ### Stag Hunt

        self.payout_mat = self.payout_mat - np.max(self.payout_mat)
